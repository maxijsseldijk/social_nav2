...
void usage()
{
  std::cout << "Bridge a collection of ROS2 and Gazebo Transport topics and services.\n\n" <<
    ...
    ".StringMsg\n" <<
    "A service bridge:\n" <<
    "    parameter_bridge /world/default/control@core_ros_gz_service_interfaces/srv/ControlWorld\n" <<
    "Or equivalently:\n" <<
    "    parameter_bridge /world/default/control@core_ros_gz_service_interfaces/srv/ControlWorld@"
    "ignition.msgs.WorldControl@ignition.msgs.Boolean\n" << std::endl;
}

using RosGzBridge = core_ros_gz_service_bridge::RosGzBridge;
using BridgeDirection = core_ros_gz_service_bridge::BridgeDirection;
using BridgeConfig = core_ros_gz_service_bridge::BridgeConfig;
  ...
  for (auto filteredArg : filteredArgs) {
    ...
    if (config.ros_type_name.find("/srv/") != std::string::npos) {
      ...
      if (config.direction == BridgeDirection::BIDIRECTIONAL) {
        delimPos = arg.find(delim);
        std::cout<<"pose: "<<delimPos<<std::endl;
        if (delimPos == std::string::npos || delimPos == 0) {
          usage();
          return -1;
        }
      ...
      std::cout<<gz_req_type_name<<std::endl;
      std::cout<<gz_rep_type_name<<std::endl;      
