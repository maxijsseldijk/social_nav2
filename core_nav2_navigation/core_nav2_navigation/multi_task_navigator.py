...
class MultiTaskNavigator(Node):
    """
    A Task Navigator based on the BasicNavigator class.

    This class includes small tweaks for use with environments that
    update the task dynamically. New tasks are requested on the
    /task_number topic. Based on the TaskGenerator parameters, the new
    task is fetched and executed.
    """

    def __init__(self, node_name='multi_task_navigator', namespace=''):
        super().__init__(
            node_name=node_name,
            namespace=namespace,
    ...
    def create_pose_from_route(self, route) -> list[PoseStamped]:
        ...
        for pt in route:
            task_pose.pose.position.x = pt[0]
            task_pose.pose.position.y = pt[1]
            task_points.append(deepcopy(task_pose))
        return task_points
    ...
    def goThroughPoses(self, poses, behavior_tree=''):
        ...
        self.debug(f'Navigating with {len(goal_msg.poses)} goals....')
    ...
    def goToPose(self, pose, behavior_tree=''):
        ...
        self.debug('Navigating to goal: ' + str(pose.pose.position.x) + ' ' +
    ...
    def cancelTask(self):
        ...
        self.debug('Canceling current task.')
